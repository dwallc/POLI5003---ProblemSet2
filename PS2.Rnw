...
<<label=Answer1>>=
m1 <- lm(obama ~ income + age + educ + female + black + dem, data=obama)
summary(m1)
@
...
@ 
I am thinking that we are supposed to interact Dem*Income and use the same control variables as answer 2. What do you guys think about this?
